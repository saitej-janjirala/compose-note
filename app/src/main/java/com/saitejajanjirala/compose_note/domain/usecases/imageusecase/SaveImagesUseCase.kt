...
object SaveImagesUseCase {
    ...
    fun imageFileUri(context: Context, uri: Uri, noteId: Int, index: Int): Uri? = try {
    ...
    operator fun invoke(context: Context, uris: List<Uri>, noteId: Int): List<Uri> {
        ...
        uris.forEachIndexed { index, uri ->
            imageFileUri(context, uri, noteId, index)?.let { imageFileUris.add(it) }
        }
